...
void FourByteTracer::CaptureStart(dev::eth::ExtVMFace const* /*ext*/, dev::Address const& /*from*/, dev::Address const& to,
                                  bool create, dev::bytes const& input, uint64_t /*gas*/, dev::u256 /*value*/)
{
    ...
    recordCallSelector(to, input);
}

void FourByteTracer::CaptureEnter(dev::eth::Instruction op, dev::Address const& /*from*/, dev::Address const& to,
...
void FourByteTracer::recordCallSelector(dev::Address const& to, dev::bytes const& input)
{
    if (input.size() < 4)
        return;

    if (dev::eth::isPrecompiledContract(to))
        return;
